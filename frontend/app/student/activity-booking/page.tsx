...
export default function StudentActivityBooking() {
  ...
  const fetchActivities = async (token: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        const signupsResponse = await axios.get('http://localhost:8080/api/reservations/signups', {
          headers: { 'x-access-token': token }
        });
        ...
        // 筛选当前时间未超过 registration_time 的活动
        const filteredByRegistrationTime = activitiesWithSignup.filter((activity: Activity) => {
          return new Date(activity.registration_time) >= currentTime;
        });

        setActivities(filteredByRegistrationTime);
      }
    } catch (error) {
  ...
  const handleSignUp = async (activityId: number) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/api/reservations/${activityId}/signup`,
        {},
        { headers: { 'x-access-token': token } }
      );
          ...
          <div>
            <Row gutter={[16, 16]}>
              ...
              {activityDetail.is_signed_up && (
                <Col span={24}>
                  <Tag color="success" style={{ padding: '5px 10px', fontSize: '14px' }}>您已成功预约此活动</Tag>
                </Col>
              )}
            </Row>
          </div>
